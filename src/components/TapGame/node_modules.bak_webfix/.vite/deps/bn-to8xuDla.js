...
var require_dist = /* @__PURE__ */ __commonJS({ "node_modules/vite-plugin-node-polyfills/shims/buffer/dist/index.cjs": ((exports) => {
	...
	var ieee754 = {};
	ieee754.read = function(buffer$2, offset, isLE, mLen, nBytes) {
		var e, m;
		...
		var d = isLE ? -1 : 1;
		var s = buffer$2[offset + i$1];
		i$1 += d;
		e = s & (1 << -nBits) - 1;
		s >>= -nBits;
		nBits += eLen;
		for (; nBits > 0; e = e * 256 + buffer$2[offset + i$1], i$1 += d, nBits -= 8);
		m = e & (1 << -nBits) - 1;
		e >>= -nBits;
		nBits += mLen;
		for (; nBits > 0; m = m * 256 + buffer$2[offset + i$1], i$1 += d, nBits -= 8);
		if (e === 0) e = 1 - eBias;
	...
	ieee754.write = function(buffer$2, value, offset, isLE, mLen, nBytes) {
		var e, m, c;
		var eLen = nBytes * 8 - mLen - 1;
		var eMax = (1 << eLen) - 1;
		var eBias = eMax >> 1;
		...
		for (; mLen >= 8; buffer$2[offset + i$1] = m & 255, i$1 += d, m /= 256, mLen -= 8);
		e = e << mLen | m;
		eLen += mLen;
		for (; eLen > 0; buffer$2[offset + i$1] = e & 255, i$1 += d, e /= 256, eLen -= 8);
		buffer$2[offset + i$1 - d] |= s * 128;
	};
	(function(exports$1) {
		...
		*/
		...
		Buffer$2.concat = function concat(list, length) {
			...
			const buffer$2 = Buffer$2.allocUnsafe(length);
			let pos = 0;
			for (i$1 = 0; i$1 < list.length; ++i$1) {
				let buf = list[i$1];
				if (isInstance(buf, GlobalUint8Array)) if (pos + buf.length > buffer$2.length) {
					if (!Buffer$2.isBuffer(buf)) buf = Buffer$2.from(buf);
					buf.copy(buffer$2, pos);
				} else GlobalUint8Array.prototype.set.call(buffer$2, buf, pos);
		...
		function bidirectionalIndexOf(buffer$2, val, byteOffset, encoding, dir) {
			if (buffer$2.length === 0) return -1;
			...
			if (numberIsNaN(byteOffset)) byteOffset = dir ? 0 : buffer$2.length - 1;
			if (byteOffset < 0) byteOffset = buffer$2.length + byteOffset;
			if (byteOffset >= buffer$2.length) if (dir) return -1;
			...
			if (Buffer$2.isBuffer(val)) {
				...
				val = val & 255;
				if (typeof GlobalUint8Array.prototype.indexOf === "function") if (dir) return GlobalUint8Array.prototype.indexOf.call(buffer$2, val, byteOffset);
				else return GlobalUint8Array.prototype.lastIndexOf.call(buffer$2, val, byteOffset);
				return arrayIndexOf(buffer$2, [val], byteOffset, encoding, dir);
			}
	...
	exports.Blob = buffer$1.Blob;
	exports.BlobOptions = buffer$1.BlobOptions;
	exports.Buffer = buffer$1.Buffer;
	exports.File = buffer$1.File;
	exports.FileOptions = buffer$1.FileOptions;
	exports.INSPECT_MAX_BYTES = buffer$1.INSPECT_MAX_BYTES;
	exports.SlowBuffer = buffer$1.SlowBuffer;
	exports.TranscodeEncoding = buffer$1.TranscodeEncoding;
	exports.atob = buffer$1.atob;
	exports.btoa = buffer$1.btoa;
	exports.constants = buffer$1.constants;
	exports.default = Buffer$1;
	exports.isAscii = buffer$1.isAscii;
	exports.isUtf8 = buffer$1.isUtf8;
	exports.kMaxLength = buffer$1.kMaxLength;
	exports.kStringMaxLength = buffer$1.kStringMaxLength;
	exports.resolveObjectURL = buffer$1.resolveObjectURL;
	exports.transcode = buffer$1.transcode;
}) });
...
var require_bn = /* @__PURE__ */ __commonJS({ "node_modules/bn.js/lib/bn.js": ((exports, module) => {
	(function(module$1, exports$1) {
		...
		var Buffer$2;
		try {
			if (typeof window !== "undefined" && typeof window.Buffer !== "undefined") Buffer$2 = window.Buffer;
			else Buffer$2 = require_dist().Buffer;
		} catch (e) {}
		...
		BN.prototype._initArray = function _initArray(number, base, endian) {
			...
			var j, w;
			var off = 0;
			if (endian === "be") for (i$1 = number.length - 1, j = 0; i$1 >= 0; i$1 -= 3) {
				w = number[i$1] | number[i$1 - 1] << 8 | number[i$1 - 2] << 16;
			...
			else if (endian === "le") for (i$1 = 0, j = 0; i$1 < number.length; i$1 += 3) {
				w = number[i$1] | number[i$1 + 1] << 8 | number[i$1 + 2] << 16;
				this.words[j] |= w << off & 67108863;
				this.words[j + 1] = w >>> 26 - off & 67108863;
				off += 24;
		...
		BN.prototype._parseHex = function _parseHex(number, start, endian) {
			...
			var j = 0;
			var w;
			if (endian === "be") for (i$1 = number.length - 1; i$1 >= start; i$1 -= 2) {
				w = parseHexByte(number, start, i$1) << off;
			...
			else for (i$1 = (number.length - start) % 2 === 0 ? start + 1 : start; i$1 < number.length; i$1 += 2) {
				w = parseHexByte(number, start, i$1) << off;
				this.words[j] |= w & 67108863;
				if (off >= 18) {
					off -= 18;
		...
		function parseBase(str, start, end, mul) {
			var r = 0;
			var b = 0;
			var len$1 = Math.min(str.length, end);
			for (var i$1 = start; i$1 < len$1; i$1++) {
		...
		BN.prototype._parseBase = function _parseBase(number, base, start) {
			...
			for (var i$1 = start; i$1 < end; i$1 += limbLen) {
				word = parseBase(number, i$1, i$1 + limbLen, base);
				this.imuln(limbPow);
			...
			if (mod !== 0) {
				var pow = 1;
				word = parseBase(number, i$1, number.length, base);
				for (i$1 = 0; i$1 < mod; i$1++) pow *= base;
				this.imuln(pow);
				if (this.words[0] + word < 67108864) this.words[0] += word;
				else this._iaddn(word);
			}
		...
		if (Buffer$2) BN.prototype.toBuffer = function toBuffer(endian, length) {
			return this.toArrayLike(Buffer$2, endian, length);
		};
		...
		BN.prototype.toArrayLike = function toArrayLike(ArrayType, endian, length) {
			this._strip();
			var byteLength$1 = this.byteLength();
			var reqLength = length || Math.max(1, byteLength$1);
			assert(byteLength$1 <= reqLength, "byte array longer than desired length");
			assert(reqLength > 0, "Requested array length <= 0");
			var res = allocate(ArrayType, reqLength);
			this["_toArrayLike" + (endian === "le" ? "LE" : "BE")](res, byteLength$1);
			return res;
		};
		BN.prototype._toArrayLikeLE = function _toArrayLikeLE(res, byteLength$1) {
			var position = 0;
			var carry = 0;
		...
		BN.prototype._toArrayLikeBE = function _toArrayLikeBE(res, byteLength$1) {
		...
		BN.prototype.byteLength = function byteLength$1() {
		...
		function smallMulTo(self, num, out) {
			...
			var len$1 = self.length + num.length | 0;
			out.length = len$1;
			len$1 = len$1 - 1 | 0;
		...
		BN.prototype.mulTo = function mulTo(num, out) {
			var res;
			var len$1 = this.length + num.length;
			if (this.length === 10 && num.length === 10) res = comb10MulTo(this, num, out);
			else if (len$1 < 63) res = smallMulTo(this, num, out);
			else if (len$1 < 1024) res = bigMulTo(this, num, out);
		...
		FFTM.prototype.convert13b = function convert13b(ws, len$1, rws, N) {
			var carry = 0;
			...
			for (i$1 = 2 * len$1; i$1 < N; ++i$1) rws[i$1] = 0;
			assert(carry === 0);
			assert((carry & -8192) === 0);
		};
		...
		K256.prototype.split = function split(input, output) {
			...
			for (i$1 = 10; i$1 < input.length; i$1++) {
				...
				input.words[i$1 - 10] = (next & mask) << 4 | prev >>> 22;
				prev = next;
			}
			prev >>>= 22;
		...
		Red.prototype.sqrt = function sqrt(a) {
			...
			while (t.cmp(one) !== 0) {
				...
				assert(i$1 < m);
				var b = this.pow(c, new BN(1).iushln(m - i$1 - 1));
				r = r.redMul(b);
				c = b.redSqr();
				t = t.redMul(c);
				m = i$1;
...
//#endregion
export { require_safe_buffer as n, require_dist as r, require_bn as t };
//# sourceMappingURL=bn-to8xuDla.js.map