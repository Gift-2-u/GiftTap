...
/** Play level from lifetime taps (same bands as GiftTap.calculateLevel). */
function playLevelFromLifetimeTaps(taps) {
...
/** Rare ≥ L5, Epic/Legendary ≥ L10. Common + Locksmith + Star always open. */
function nftMintBlockedByPlayLevel(playLevel, rarityKey, kindKey = '') {
  const kind = String(kindKey || '')
    .toLowerCase()
    .replace(/\s+/g, '');
  if (kind.includes('locksmith') || kind.includes('star') || kind === 'shard_badge') {
    return null;
  }
...
const Marketplace = ({ balance, setBalance, stats, setStats, setEnergy, bumpEnergyEpoch, flushPendingTaps, player, tgUser, playerWallet, decryptedPhrase, initialTab, onInitialTabConsumed, maxUnlockedLevel = 4, lifetimeTaps = null, onChainBalanceChange = null, onMaxDailyLimitChange = null }) => {
  ...
  // Free shop = badges only (Bronze→Diamond). Frenzy/Battery/Refill not for sale.
  const showBadgeShop = true;
  const [activeTab, setActiveTab] = useState(initialTab || 'home');

  // Deep-link: Free badges tab, or Premium
  useEffect(() => {
  ...
  // Play level from lifetime taps (not wall) — gates Rare / Epic / Legendary mints
  const playLevel = playLevelFromLifetimeTaps(
  ...
  // Free boost defs ALWAYS kept for Backpack (Mystery / leftovers). Not sold in Free shop.
  ...
  const freeBadgeShopListings = [
    ...
    {
      id: 'badge_gold',
      name: BADGE_SHARD_SHOP.badge_gold.name,
      desc: 'For Mystery Gift · 1/day · 3/week',
      duration: 'Keep in Backpack',
      cost: BADGE_SHARD_SHOP.badge_gold.cost,
      isBadgeShop: true,
      iconUrl: BADGE_TIERS.gold.image,
      iconFrom: '#fbbf24',
      iconTo: '#854d0e',
      iconRing: 'rgba(251,191,36,0.5)',
      iconGlow: 'rgba(251,191,36,0.25)',
    },
    {
      id: 'badge_diamond',
      name: BADGE_SHARD_SHOP.badge_diamond.name,
      desc: 'For Mystery Gift · 1/day · 3/week',
      duration: 'Keep in Backpack',
      cost: BADGE_SHARD_SHOP.badge_diamond.cost,
      isBadgeShop: true,
      iconUrl: BADGE_TIERS.diamond.image,
      iconFrom: '#67e8f9',
      iconTo: '#0e7490',
      iconRing: 'rgba(103,232,249,0.5)',
      iconGlow: 'rgba(34,211,238,0.25)',
    },
  ];
  // Free shop sells badges only
  const shardListings = freeBadgeShopListings;
  ...
  const handleFateMint = async (rarityKey) => {
    ...
    {
      const blocked = nftMintBlockedByPlayLevel(playLevel, rarityKey, 'fate');
      if (blocked) {
        setTxStatus({ show: true, loading: false, message: `❌ ${blocked}`, success: false });
        return;
      }
    }
...
Luck jackpot active · Pack → NFT to see it.`,
  ...
  const handleEchoMint = async (rarityKey) => {
    ...
    {
      const blocked = nftMintBlockedByPlayLevel(playLevel, rarityKey, 'echo');
      if (blocked) {
        setTxStatus({ show: true, loading: false, message: `❌ ${blocked}`, success: false });
        return;
      }
...
Tap multi active · Pack → NFT to see it.`,
  ...
  const handleRushMint = async (rarityKey) => {
    ...
    {
      const blocked = nftMintBlockedByPlayLevel(playLevel, rarityKey, 'rush');
      if (blocked) {
        setTxStatus({ show: true, loading: false, message: `❌ ${blocked}`, success: false });
        return;
      }
...
Daily cap active · Pack → NFT to see it.`,
  ...
  const handleShadowMint = async (rarityKey) => {
    ...
    {
      const blocked = nftMintBlockedByPlayLevel(playLevel, rarityKey, 'shadow');
      if (blocked) {
        setTxStatus({ show: true, loading: false, message: `❌ ${blocked}`, success: false });
        return;
      }
  ...
  return (
    <div style={{ flex: 1, width: '100%', display: 'flex', flexDirection: 'column', padding: '15px', paddingBottom: '120px', boxSizing: 'border-box' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        ...
        {/* --- HUB: Badges (Free) / Premium / NFT / Backpack --- */}
        {activeTab === 'home' && (
          <>
            <p style={{ color: '#888', fontSize: 12, margin: '0 0 12px', textAlign: 'center', lineHeight: 1.45 }}>
              Badges with shards, Premium boosts, NFTs, or your backpack. Free Energy is on the tap screen (ads).
            </p>
            <div
              style={{
                display: 'grid',
        ...
        {/* --- Free shop: badges only --- */}
        {activeTab === 'upgrades' && showBadgeShop && (
          <>
            <p style={{ color: '#666', fontSize: 11, margin: '0 0 12px', textAlign: 'center' }}>
              Free · buy badges with G2Ushards · burn in Mystery Gift
            </p>
            <p
              style={{
                color: '#888',
            ...
            >
              Bronze 10k · Silver 30k · Gold 50k · Diamond 75k · max {BADGE_SHOP_DAY_CAP}/day ·{' '}
              {BADGE_SHOP_WEEK_CAP}/week (any tier). Weekly ranks still award badges too.
            </p>
                ...
                nftDetail.isShadowMint ? (
                  ...
                  >
                    ...
                    <div style={{ color: '#666', fontSize: 10, lineHeight: 1.35 }}>
                      Starts at 100% when owned · drains 1% / 2,000 shards · off at 0% ·
                      reload with $G2U in Wallet / Backpack (50 G2U = 1%)
                    </div>
                  </div>
                ) : null}
              </div>
