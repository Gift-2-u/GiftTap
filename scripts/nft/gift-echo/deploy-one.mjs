/**
 * Deploy one Echo Wave 1 CM + Guard + Wrap with priority fees.
...
const TREASURY = publicKey(process.env.TREASURY || ECHO_TREASURY);
...
console.log("=== Echo", r.label, "Wave 1 ===");
...
let imagePath = path.join(__dirname, `Echo-${rarityKey}.jpg`);
if (!fs.existsSync(imagePath)) imagePath = path.join(__dirname, "Echo.jpg");
