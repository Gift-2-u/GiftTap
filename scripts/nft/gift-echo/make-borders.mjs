...
const srcPath = path.join(dir, "Echo.jpg");
if (!fs.existsSync(srcPath)) throw new Error("Missing Echo.jpg");
...
for (const [name, { border: borderColor }] of Object.entries(colors)) {
  ...
  const p = path.join(dir, `Echo-${name}.jpg`);
...
const pubDir = path.join(dir, "../../../public/nft/echo");
fs.mkdirSync(pubDir, { recursive: true });
for (const name of Object.keys(colors)) {
  const src = path.join(dir, `Echo-${name}.jpg`);
  const dst = path.join(pubDir, `Echo-${name}.jpg`);
  fs.copyFileSync(src, dst);
  console.log("public", path.basename(dst));
}

console.log("done — Echo borders + locked socket (from socket-geometry.mjs)");
