...
const srcPath = path.join(dir, "Shadow.jpg");
if (!fs.existsSync(srcPath)) throw new Error("Missing Shadow.jpg");
...
for (const [name, { border: borderColor }] of Object.entries(colors)) {
  ...
  const p = path.join(dir, `Shadow-${name}.jpg`);
...
const pubDir = path.join(dir, "../../../public/nft/shadow");
fs.mkdirSync(pubDir, { recursive: true });
for (const name of Object.keys(colors)) {
  const src = path.join(dir, `Shadow-${name}.jpg`);
  const dst = path.join(pubDir, `Shadow-${name}.jpg`);
  fs.copyFileSync(src, dst);
  console.log("public", path.basename(dst));
}

console.log("done — Shadow borders + locked socket (from socket-geometry.mjs)");
