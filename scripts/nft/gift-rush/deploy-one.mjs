/**
 * Deploy one Rush Wave 1 CM + Guard + Wrap with priority fees.
...
const TREASURY = publicKey(process.env.TREASURY || RUSH_TREASURY);
...
console.log("=== Rush", r.label, "Wave 1 ===");
...
let imagePath = path.join(__dirname, `Rush-${rarityKey}.jpg`);
if (!fs.existsSync(imagePath)) imagePath = path.join(__dirname, "Rush.jpg");
