...
async function main() {
  ...
  const imagePath = path.join(__dirname, 'StarBadge.jpg');
  if (!fs.existsSync(imagePath)) throw new Error('Missing StarBadge.jpg');

  console.log('=== Star Badge Wave 1 setup (Metaplex Core) ===');
  ...
  if (!collectionUri) {
    const collectionMeta = {
      ...
      symbol: 'ELVES',
      description:
        'Gift2u Elves — Star Badge sockets into Fate/Echo/Rush/Shadow (UI outside art). Open mintable.',
      image: imageUri,
  ...
  if (!itemUri) {
    const itemMeta = {
      ...
      symbol: 'Locksmith',
      description:
        'Star Badge — socket outside elf art in Backpack. One Star for Fate/Echo/Rush/Shadow. Levels L1–L5 in SOL.',
      image: imageUri,
      external_url: 'https://gift2u.fun',
      attributes: [
  ...
  // Always reuse Gift2u Elves collection unless COLLECTION_ADDRESS overrides
  ...
  const pubCm = path.join(__dirname, '../../../public/star-cm.json');
  fs.writeFileSync(
    pubCm,
    JSON.stringify(
      {
        candyMachine: out.candyMachine,
        candyGuard: out.candyGuard,
        collection: out.collection,
        treasury: out.treasury || umi.identity.publicKey,
        priceSol: PRICE_SOL,
        feeBufferSol: 0.02,
        maxPerWallet: 10,
        itemsAvailable: ITEMS,
        ...
        imageUri: out.imageUri || imageUri,
      },
      null,
      2,
    ) + '\n',
  );
  console.log('Wrote', pubCm);
